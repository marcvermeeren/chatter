...
function cmdSend(me, args) {
    ...
    if (!rest[0] || !body)
        die('usage: chatter send <agent> <message...> [--queue]');
    const to = resolveRecipient(rest[0], { allowUnknown: queue });
    if (to === me.name)
        die('cannot message yourself');
...
function cmdPost(me, args) {
    ...
    if (!body)
        die('usage: chatter post <text...>   (mention with @name; @everyone is human-only)');
...
function cmdChat(me, args) {
    ...
    emit(rows, () => {
        ...
        for (const m of rows)
            console.log(`#${m.id} ${m.created_at} ${m.from_agent}: ${clean(m.body)}`);
    });
...
function identity(name, role) {
    ...
    if (!label || sanitizeName(label) === name)
        return `@${name}`;
    return `${label} · @${name}`;
}
function cmdAgents(me, args = []) {
    ...
    const registered = db().prepare(all ? 'SELECT * FROM agents ORDER BY name' : 'SELECT * FROM agents WHERE departed_at IS NULL ORDER BY name').all();
    ...
    emit(rows, () => {
        ...
        const lines = rows.map((a) => row(a.name === me.name ? '*' : ' ', identity(a.name, a.role), a.status, a.branch, a.task ? `${a.task.id} ${a.task.title}` : ''));
        for (const l of live) {
            if (l.name && !known.has(l.name))
                lines.push(row(' ', `@${l.name}`, l.agent_status, null, '(not yet on chatter)'));
            if (!l.name)
                lines.push(row(' ', 'pane:' + l.pane_id, l.agent_status, null, `(unnamed ${l.agent || 'agent'})`));
        }
...
function cmdNote(me, args) {
    ...
    if (!text)
        die('usage: chatter note <text> [--type discovery|decision|dead-end] [--task TASK-n] [--commit SHA]');
...
function cmdAsk(me, args) {
    if (!args.length)
        die('usage: chatter ask [agent] <question...>');
...
function cmdQuestions(_me, args) {
    ...
    if (!opts.all) {
        ...
        emit(rows, () => {
            ...
            for (const r of rows)
                console.log(`#${r.id} (${age(r.created_at)} old) ${r.author}: ${clean(r.text)}  (answer: chatter answer ${r.id} "...")`);
        });
        return;
    }
    ...
    emit(rows.map((r) => ({ ...r, answer: answers[r.superseded_by] || null })), () => {
        ...
        for (const r of rows) {
            ...
            if (a)
                console.log(`    -> ${a.author}: ${clean(a.text).replace(/^answer to #\d+: /, '')}`);
        }
    });
}
// ------------------------------------------------------------------- tasks
...
function cmdTask(me, args) {
    ...
    if (sub === 'create') {
        ...
        if (!title)
            die('usage: chatter task create <title> [--assignee agent]');
        ...
        if (assignee)
            logEvent(me.name, 'task_assigned', id, { to: assignee });
        ...
        if (!id || !agent)
            die('usage: chatter task assign <TASK-n> <agent>');
        ...
        const id = opts._[0];
        if (!id)
            die('usage: chatter task done <TASK-n> [--commit SHA]');
...
function cmdHandoff(me, args) {
    if (args[0] === 'show') {
        ...
        if (!h)
            die(`handoff h${args[1]} not found`);
    ...
    if (!task)
        die(`${taskId} not found — create it first: chatter task create <title>`);
...
function briefWindow(me, d, arg) {
    ...
    if (arg)
        die(`usage: chatter brief [today|2h|30m] — got "${arg}"`);
...
function repoUniverses() {
    return listRepoDbFiles().map((f) => {
        ...
        for (const suffix of ['', '-wal', '-shm']) {
            try {
                bytes += fs.statSync(f + suffix).size;
            }
            catch { /* absent */ }
        }
        return {
...
function humanOnly(me, what) {
    if (!me.human)
        die(`${what} is human-only — agents administer nothing outside their repo`);
}
function cmdData(me) {
...
function cmdPurge(me, args) {
    ...
    if (opts.all)
        ...
        if (!m)
            die('usage: chatter purge --older-than <Nd|Nh> [--yes]  (trims old messages+events in the current repo)');
        ...
        if (!targets.length)
            die(`no stored universe matches "${opts._[0]}" — see: chatter data`);
    }
    else {
...
function cmdRole(me, args) {
    ...
    const text = args.slice(1).join(' ').trim();
    if (!target || !text)
        die('usage: chatter role <agent> <display role...>   e.g. chatter role data-api "Data / API"');
...
function cmdForget(me, args) {
    ...
    if (!name)
        die('usage: chatter forget <agent>');
    const d = db();
    const row = d.prepare('SELECT name, departed_at FROM agents WHERE name = ?').get(name);
    if (!row)
        die(`no roster entry for "${name}" — see: chatter agents --all`);
    if (!row.departed_at) {
        d.prepare('UPDATE agents SET departed_at = ? WHERE name = ?').run(now(), name);
