...
function initialDbFile() {
    ...
    if (g.repoRoot && fs.existsSync(repoDbFile(g.repoRoot)))
...
function highlightMentions(line, human) {
    return line.replace(/@([a-z0-9_-]+)/g, (_, n) => (n === human || n === 'everyone')
        ? `${T.INV}@${n}${T.RESET}`
...
function runSlash(body, d, ui, paint) {
    ...
    if (cmd === 'brief') {
        try {
            ...
            ui.block = { title: 'brief', lines: [String(e.message)] };
        }
        return;
    }
...
function renderBoard(d, file, files) {
    ...
    for (const m of msgs)
        out.push(`  ${T.CHROME}${localHM(m.created_at)}${T.RESET} ${T.author(m.from_agent)}: ${highlightMentions(T.clean(m.body), humanName())}`.slice(0, width + 60));
    out.push('', ` ${T.BOLD}Tasks${T.RESET}`);
    if (!tasks.length)
        out.push(`   ${T.FAINT}(none)${T.RESET}`);
    for (const t of tasks)
        out.push('  ' + taskLabel(t));
...
function runView(render, { input = false } = {}) {
    ...
    const paint = () => {
        ...
        if (input) {
            ...
            for (const a of teamAgents(d, { fresh: true }))
                if (a.name)
                    set.add(a.name);
            ui.names = [...set].sort();
        }
            ...
            switch (key.type) {
                ...
                case 'tab': {
                    ...
                    if (m) {
                        ...
                        if (hits.length)
                            ui.buffer = ui.buffer.slice(0, ui.buffer.length - m[1].length) + hits[0] + ' ';
                    }
                    return paint();
                }
                case 'backspace':
