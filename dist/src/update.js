...
function updateStatus({ source, root }, { timeout = 3000 } = {}) {
    ...
    if (src.kind === 'github') {
        if (!src.owner || !src.repo)
            return { state: 'unknown', reason: 'the registry has no owner/repo for this install' };
        const remote = remoteHead({ url: cloneUrl(src), timeout });
        if (!remote)
            return { state: 'unknown', reason: 'could not reach GitHub' };
        return { state: sameCommit(src.resolved_commit, remote) ? 'current' : 'behind' };
    }
    if (src.kind === 'local') {
        const local = git(['rev-parse', 'HEAD'], { cwd: root }).out;
        if (!local)
            return { state: 'unknown', reason: `${root} is not a git checkout` };
        const remote = remoteHead({ cwd: root, timeout });
        if (!remote)
            return { state: 'unknown', reason: 'could not reach the remote' };
        if (sameCommit(local, remote))
...
function runUpdate({ source, root }, { check = false } = {}) {
    ...
    if (src.kind !== 'github' && src.kind !== 'local') {
        return fail('chatter is not registered with Herdr', 'install it:  herdr plugin install <owner>/<repo>', 'or link this checkout:  herdr plugin link <path>');
    }
    if (check) {
        const st = updateStatus({ source, root }, { timeout: 10000 });
        if (st.state === 'unknown')
            return fail(`could not check for updates — ${st.reason}`);
    ...
    if (src.kind === 'github') {
        if (!src.owner || !src.repo) {
            return fail('the registry has no owner/repo for this install', 'reinstall by hand:  herdr plugin install <owner>/<repo> --yes');
        }
        ...
        if (fresh && fresh.plugin_root)
            rootAfter = fresh.plugin_root;
    }
    else {
        const status = git(['status', '--porcelain'], { cwd: root });
        if (!status.ok) {
            return fail(`${root} is not a git checkout — nothing to pull`, 'if you moved it, re-link it:  herdr plugin link <path>');
        }
        if (status.out) {
            return fail(`${root} has uncommitted changes — commit or stash them first, then rerun`, 'chatter will not discard your work');
        }
